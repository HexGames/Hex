...
{
    internal static class BonusStepBuffer
    {
        internal struct BonusStepList
        {
            public int Start;
            public int Count;
            public int Capacity;

            public BonusStepList(int start, int count, int capacity)
            {
                Start = start;
                Count = count;
                Capacity = capacity;
        ...
        private static BonusStep[] _buffer;
        private static BonusStepList[] _lists;
        ...
        internal static void CreateList(out int listID, int initialCapacity = CHUNK)
        {
            ...
            _lists[_listsCount] = new BonusStepList(_usedCapacity, 0, capacity);
        ...
        internal static void Add(BonusStepListRef handle, in BonusStep step)
        {
        ...
        internal static ref BonusStep Get(BonusStepListRef listRef, int index)
        {
            ref BonusStepList list = ref _lists[listRef.Id];
        ...
        internal static int Count(BonusStepListRef handle) => _lists[handle.Id].Count;

        private static void Grow(BonusStepListRef handle)
        {
            ref BonusStepList list = ref _lists[handle.Id];
        ...
        private static void EnsureBufferCapacity(int requiredCapacity)
        {
            if (_buffer == null)
            {
                _buffer = new BonusStep[Math.Max(INITIAL_BUFFER, requiredCapacity)];
            }
            ...
            var newBuffer = new BonusStep[capacity];
        ...
        {
            if (_lists == null)
                _lists = new BonusStepList[INITIAL_ENTRIES];
        ...
        internal static void Clear(BonusStepListRef handle)
        {
            ref BonusStepList list = ref _lists[handle.Id];
